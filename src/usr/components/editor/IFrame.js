...
class IFrame extends React.Component {
  ...
  shouldComponentUpdate (nextProps, nextState, nextContent) {
    const { url, width, scale, offPointers } = this.props;
    const { url: nextUrl, width: nextWidth, scale: nextScale, offPointers: nextOffPointers } = nextProps;
    return url !== nextUrl || width !== nextWidth || scale !== nextScale || offPointers !== nextOffPointers;
  }
  ...
  render () {
    ...
    if (offPointers) {
      iFrameStyle = {...iFrameStyle, pointerEvents: 'none'};
    }
    if (scale < 1) {
      iFrameStyle = {
        ...iFrameStyle,
