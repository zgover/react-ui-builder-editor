...
class LivePreview extends React.Component {
  ...
  componentDidMount () {
    ...
    const { demoFiles } = this.props;
    if (demoFiles) {
      setTimeout(() => {
        if (this.iFrameRef.current) {
          this.iFrameRef.current.sendMessage({
            type: constants.WEBCODESK_MESSAGE_SAVE_DEMO_FILES,
            payload: demoFiles,
          });
        }
      }, 2000);
  ...
  shouldComponentUpdate (nextProps, nextState, nextContext) {
    const { isVisible, pages, appUrl, demoFiles } = this.props;
    const {
      iFrameWidthIndex,
      iFrameScaleIndex,
    ...
    return iFrameWidthIndex !== nextState.iFrameWidthIndex
      ...
      || appUrl !== nextProps.appUrl
      || demoFiles !== nextProps.demoFiles;
  }

  componentDidUpdate (prevProps, prevState, snapshot) {
    const { demoFiles } = this.props;
    if (demoFiles && demoFiles !== prevProps.demoFiles) {
      if (this.iFrameRef.current) {
        this.iFrameRef.current.sendMessage({
          type: constants.WEBCODESK_MESSAGE_SAVE_DEMO_FILES,
          payload: demoFiles,
  ...
  handleFrameworkMessage = (message) => {
    if (message) {
      ...
      } else if (type === constants.FRAMEWORK_MESSAGE_DEMO_FILES_SAVED) {
        this.handleReload();
      }
    }
  };
  ...
  handleChangeActiveUrl = (url) => {
    ...
    if(this.iFrameRef.current) {
      this.iFrameRef.current.loadURL(`${appUrl}${url}`);
    }
  };
  ...
  handleFrameReady = (url) => {
    const { appUrl } = this.props;
    this.setState({
      frameUrl: url ? url.replace(`${appUrl}`, '') : '',
    });
  };

  handleOpenExternal = () => {
    const { appUrl } = this.props;
    this.props.onOpenUrl(`${appUrl}${this.state.frameUrl}`);
  };
